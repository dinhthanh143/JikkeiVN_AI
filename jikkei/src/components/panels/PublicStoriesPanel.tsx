...
import { fetchPublicScenes, type SceneDetailRecord } from '@/services/backendApi'
...
function StoryModal({ story, onClose, onPlay, onAuthorClick }: StoryModalProps) {
        ...
        <div className="story-modal-body">
          ...
          <div className="story-modal-info">
            ...
            <div className="story-modal-actions">
              ...
              <button type="button" className="jk-modal-btn" onClick={onClose}>
                CLOSE
              </button>
            </div>
          </div>
...
export function PublicStoriesPanel() {
  ...
  // Debounce raw search input -> only update `search` (which triggers the query) after 300ms idle
  useEffect(() => {
  ...
  const {
    ...
    queryKey: ['publicScenes', queryParams],
    queryFn: () => fetchPublicScenes(queryParams),
  })

  // Infer next page existence from result count
  useEffect(() => {
    setHasNextPage(publicScenes.length === PAGE_SIZE)
  }, [publicScenes])
  ...
  return (
    <>
      <div className="panel-content play-panel-content">
        <div className="play-toolbar">
          <div className="play-search-wrap">
            ...
            />
            {searchInput && (
              <button type="button" className="play-search-clear" onClick={() => setSearchInput('')}>
                ✕
              </button>
            )}
          </div>
        ...
        <section className="play-section">
          <div className="play-section-header">
            <div>
              <span className="panel-eyebrow">// COMMUNITY_PICKS</span>
              <h2 className="panel-title">Public Stories</h2>
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="play-empty-state">
              ...
              <p className="play-empty-text">Loading public stories...</p>
            </div>
          ) : error ? (
            <div className="play-empty-state">
              <span className="play-empty-icon">✗</span>
            ...
            filtersActive ? (
              ...
              <div className="play-empty-state">
                <span className="play-empty-icon">⬡</span>
                <p className="play-empty-text">No public stories available yet.</p>
              </div>
            )
          ) : (
            <>
              <div className="story-grid">
                {publicScenes.map((story) => (
      ...
      <style>{`
        ...
        .story-card-title { font-family: var(--font-display, 'Comfortaa', sans-serif); font-size: 0.75rem; font-weight: 700; color: #fff; letter-spacing: 0.02em; line-height: 1.3; margin: 0 0 2px; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
        .story-card-author { font-family: var(--font-ui, sans-serif); font-size: 0.65rem; color: rgba(255, 211, 225, 0.75); margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
